...
async function saveToResourceManager(instanceId) {
    ...
    try {
        ...
        if (instanceHasFssVolumes && configuredFileStorages.length === 0) {
            showNotification('Resource Manager export needs the File System definition (mount path, mount target and export OCIDs) to recreate this FSS volume. Add the missing definition before exporting.', 'error', 10000);
            return;
        }

        if (hasIncompleteFileStorageDefinitions(configuredFileStorages)) {
            showNotification('Each File System needs a mount path, mount target OCID and export OCID before it can be exported to Resource Manager.', 'error', 10000);
            return;
        }

        for (const [index, fileStorage] of configuredFileStorages.entries()) {
            const mountOptionsError = validateFssMountOptions(fileStorage.mountOptions);
            if (mountOptionsError) {
                showNotification(`File System ${index + 1} has invalid mount options: ${mountOptionsError}`, 'error', 10000);
                return;
            }
        }
        ...
        const containersConfig = containers.map((container, idx) => {
            ...
            const emptyDirMounts = volumes.map((v, volIdx) => ({
                mount_path: v.path,
                volume_name: v.name || `volume-${volIdx}`
            }));
            const fssMounts = buildFileStorageVolumeMounts(configuredFileStorages).map((mount) => ({
                mount_path: mount.mountPath,
                volume_name: mount.volumeName,
                sub_path: mount.subPath,
                is_read_only: mount.isReadOnly
            }));

            if (emptyDirMounts.length > 0 || fssMounts.length > 0) {
                containerConfig.volume_mounts = [...emptyDirMounts, ...fssMounts];
            }
        ...
        const fssVolumesConfig = configuredFileStorages.map((fileStorage, idx) => ({
            name: getFileStorageName(fileStorage, idx),
            volume_type: 'OCI_FSS_FILE_SYSTEM',
            mount_target_id: fileStorage.mountTargetId,
            export_id: fileStorage.exportId,
            subnet_id: fileStorage.subnetId?.trim() || '',
            mount_options: normalizeMountOptions(fileStorage.mountOptions),
            is_encrypted_in_transit: fileStorage.isEncryptedInTransit !== false
        }));
...
terraform {
  required_providers {
    oci = {
      source  = "oracle/oci"
      # FSS Container Instance volume support is available in this provider version.
      version = ">= 7.25.0"
    }
  }
}
...
${containersConfig.map((container, idx) => {
    ...
    if (container.volume_mounts && container.volume_mounts.length > 0) {
        containerBlock += `\n${container.volume_mounts.map(vm => `    volume_mounts {\n      mount_path  = "${vm.mount_path}"\n      volume_name = "${vm.volume_name}"${vm.sub_path ? `\n      sub_path    = "${vm.sub_path}"` : ''}${typeof vm.is_read_only === 'boolean' ? `\n      is_read_only = ${vm.is_read_only}` : ''}\n    }`).join('\n')}`;
    }
...
${fssVolumesConfig.length > 0 ? fssVolumesConfig.map(vol => `  volumes {
    name        = "${vol.name}"
    volume_type = "${vol.volume_type}"

    mount_target {
      id                        = "${vol.mount_target_id}"
...
${vol.mount_options.length > 0 ? `
    mount_command {
${vol.mount_options.map(option => `      mount_options {
...
` : ''}
    security {
      auth                    = "SYS"
      is_encrypted_in_transit = ${vol.is_encrypted_in_transit}
    }
${vol.subnet_id ? `
    subnet_id = "${vol.subnet_id}"
` : ''}  }`).join('\n\n') : ''}
