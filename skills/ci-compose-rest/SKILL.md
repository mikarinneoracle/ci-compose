...
- **Update:** first retrieve the existing Container Instance. Build, validate, and show the complete replacement payload and its tags. Explain that the replacement is created first with the same display name while the original remains running. Obtain explicit confirmation before making changes. Then fetch the selected configuration's latest revision and persist the exact replacement `projectResources` with `PUT /api/configs/:configId` **before** `POST /api/oci/container-instances`, so the UI reflects the planned replacement before the OCI deployment changes. If this configuration save conflicts or otherwise fails, stop and leave the original untouched. If the create request fails after the configuration save, leave the original untouched and report that the UI reflects the intended replacement while the current deployment remains in place. After a successful create response, delete the original immediately with `DELETE /api/oci/container-instances/:instanceId`; do not ask for or wait for a visibility confirmation, and do not poll, wait for readiness, or test the new instance.
...
Do not restart or stop a Container Instance as a substitute for an update. Do not delete a deployment except after its replacement configuration has been saved successfully and the replacement create request succeeded, or after explicit confirmation for a standalone delete request.
